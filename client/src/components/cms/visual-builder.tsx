...
const colorPresets = [
  { name: 'Primær', value: 'hsl(var(--primary))', swatchClass: 'bg-primary' },
  { name: 'Sekundær', value: 'hsl(var(--secondary))', swatchClass: 'bg-secondary' },
  { name: 'Aksent', value: 'hsl(var(--accent))', swatchClass: 'bg-accent' },
  { name: 'Bakgrunn', value: 'hsl(var(--background))', swatchClass: 'bg-background' },
  { name: 'Muted', value: 'hsl(var(--muted))', swatchClass: 'bg-muted' },
];
...
function LayerItem({ label, icon: Icon, type, id, isVisible = true, children, depth = 0 }: LayerItemProps) {
  ...
  const depthPaddingClass = depth === 0
    ? 'pl-2'
    : depth === 1
      ? 'pl-6'
      : depth === 2
  ...
  return (
    <div className="select-none">
      <div
        className={`flex items-center gap-1 py-1.5 cursor-pointer rounded-md transition-colors ${depthPaddingClass} ${
  ...
  return (
    <div className="h-full flex flex-col bg-muted/30">
      <div className="flex-1 overflow-auto p-4 flex justify-center">
        <div 
          className={`bg-background shadow-lg rounded-lg overflow-hidden transition-all origin-top ${zoomScaleClass} ${deviceStyles[deviceMode]}`}
        >
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 flex-wrap">
              {colorPresets.map((color) => (
                <button
                  key={color.name}
                  className={`w-6 h-6 rounded border-2 ${color.swatchClass} ${bgColor === color.value ? 'border-primary' : 'border-transparent'}`}
          ...
          {Object.entries(colors).map(([key, value]) => (
            <div key={key} className="flex items-center gap-3">
              <Input
                ...
                title={`Velg ${key} farge`}
                aria-label={`Velg ${key} farge`}
  ...
  return (
    <div className="p-4 space-y-4">
      <input
        ...
        accept="image/*"
        className="hidden"
        title="Last opp mediafil"
        aria-label="Last opp mediafil"
