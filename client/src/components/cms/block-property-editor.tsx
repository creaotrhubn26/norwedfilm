...
function ImageGalleryEditor({ content, onUpdate }: { content: any; onUpdate: ContentUpdater }) {
  ...
  const updateAnimationAdvanced = (updates: Record<string, any>) => {
    onUpdate({
      ...content,
      imageAnimationAdvanced: {
        ...animationAdvanced,
        ...updates,
      },
    });
  ...
  const animationPresets = {
    filmNoir: {
      label: 'Film noir',
      imageAnimation: 'cinematic-pan',
      imagePresentation: 'cinematic',
      imageAnimationAdvanced: {
        durationSec: 18,
        delayStepMs: 180,
    ...
    romantic: {
      label: 'Romantic',
      imageAnimation: 'soft-float',
      imagePresentation: 'soft',
      imageAnimationAdvanced: {
        durationSec: 10,
        delayStepMs: 120,
        easing: 'ease-in-out',
    ...
    documentary: {
      label: 'Documentary',
      imageAnimation: 'cinematic-reveal',
      ...
      imageAnimationAdvanced: {
        ...
        revealYOffsetPx: 10,
        revealScaleStart: 1.01,
      },
    },
  } as const;

  const applyAnimationPreset = (presetKey: keyof typeof animationPresets) => {
    const preset = animationPresets[presetKey];
    onUpdate({
      ...content,
      imageAnimation: preset.imageAnimation,
      imagePresentation: preset.imagePresentation,
      imageAnimationAdvanced: {
        ...animationAdvanced,
        ...preset.imageAnimationAdvanced,
      },
    });
  };

  const activeAnimationPreset = (Object.entries(animationPresets) as Array<[keyof typeof animationPresets, (typeof animationPresets)[keyof typeof animationPresets]]>)
    .find(([, preset]) => {
      if (content?.imageAnimation !== preset.imageAnimation) return false;
      if (content?.imagePresentation !== preset.imagePresentation) return false;

      return Object.entries(preset.imageAnimationAdvanced).every(([key, value]) => {
        return animationAdvanced[key as keyof typeof animationAdvanced] === value;
      });
    })?.[0] ?? null;

  const nearestAnimationPreset = (Object.entries(animationPresets) as Array<[keyof typeof animationPresets, (typeof animationPresets)[keyof typeof animationPresets]]>)
    .find(([, preset]) => {
      return content?.imageAnimation === preset.imageAnimation && content?.imagePresentation === preset.imagePresentation;
    })?.[0] ?? null;

  const isPresetCustomized = Boolean(nearestAnimationPreset) && !activeAnimationPreset;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        <Label className="text-xs">Bildefit</Label>
        <Select value={content?.imageFit || 'cover'} onValueChange={v => onUpdate({ ...content, imageFit: v })}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="cover">Cover (fyll kort)</SelectItem>
            <SelectItem value="contain">Contain (vis hele bilde)</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <div className="space-y-1">
        <Label className="text-xs">Bildeformat</Label>
        <Select value={content?.aspectRatio || '4:3'} onValueChange={v => onUpdate({ ...content, aspectRatio: v })}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-1">
        <Label className="text-xs">Presentasjon</Label>
        <Select value={content?.imagePresentation || 'standard'} onValueChange={v => onUpdate({ ...content, imagePresentation: v })}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="standard">Standard</SelectItem>
            <SelectItem value="cinematic">Cinematisk</SelectItem>
            <SelectItem value="soft">Soft / Romantisk</SelectItem>
            <SelectItem value="editorial">Editorial</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <div className="space-y-1">
        <Label className="text-xs">Animasjon</Label>
        <Select value={content?.imageAnimation || 'none'} onValueChange={v => onUpdate({ ...content, imageAnimation: v })}>
          <SelectTrigger><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">Ingen</SelectItem>
            <SelectItem value="cinematic-pan">Cinematic Pan (Ken Burns)</SelectItem>
            <SelectItem value="cinematic-reveal">Cinematic Reveal</SelectItem>
            <SelectItem value="soft-float">Soft Float</SelectItem>
          </SelectContent>
        </Select>
      </div>
      <div className="space-y-1">
        ...
        <div className="grid grid-cols-3 gap-1">
          <Button
            type="button"
            variant={activeAnimationPreset === 'filmNoir' ? 'default' : 'outline'}
            size="sm"
            className="h-7 px-2 text-[11px]"
            onClick={() => applyAnimationPreset('filmNoir')}
          >
            Film noir
          </Button>
          <Button
            type="button"
            variant={activeAnimationPreset === 'romantic' ? 'default' : 'outline'}
            size="sm"
            className="h-7 px-2 text-[11px]"
            onClick={() => applyAnimationPreset('romantic')}
          >
            Romantic
          </Button>
          <Button
            type="button"
            variant={activeAnimationPreset === 'documentary' ? 'default' : 'outline'}
            size="sm"
            className="h-7 px-2 text-[11px]"
            onClick={() => applyAnimationPreset('documentary')}
          >
            Documentary
          </Button>
        </div>
      </div>
      {content?.imageAnimation && content.imageAnimation !== 'none' && (
        <div className="space-y-2 rounded-md border p-3 bg-muted/20">
          <Label className="text-xs font-medium">Avanserte animasjonsinnstillinger</Label>

          <div className="grid grid-cols-2 gap-2">
            <div className="space-y-1">
              <Label className="text-[11px] text-muted-foreground">Varighet (sek)</Label>
              <Input
                type="number"
                step="0.1"
                min="0.2"
                value={animationAdvanced.durationSec}
                onChange={e => updateAnimationAdvanced({ durationSec: Number(e.target.value || 14) })}
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                min="0"
                value={animationAdvanced.delayStepMs}
                onChange={e => updateAnimationAdvanced({ delayStepMs: Number(e.target.value || 120) })}
                className="h-8 text-xs"
              />
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <Select value={animationAdvanced.easing} onValueChange={v => updateAnimationAdvanced({ easing: v })}>
              <SelectTrigger className="h-8"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="ease-in-out">ease-in-out</SelectItem>
                <SelectItem value="ease">ease</SelectItem>
                <SelectItem value="linear">linear</SelectItem>
                <SelectItem value="cubic-bezier(0.2, 0.8, 0.2, 1)">cinematic-bezier</SelectItem>
                <SelectItem value="cubic-bezier(0.25, 0.46, 0.45, 0.94)">smooth-bezier</SelectItem>
              </SelectContent>
            </Select>
          </div>

          {content?.imageAnimation === 'cinematic-pan' && (
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Scale start</Label>
                <Input type="number" step="0.01" min="1" value={animationAdvanced.panScaleStart} onChange={e => updateAnimationAdvanced({ panScaleStart: Number(e.target.value || 1.02) })} className="h-8 text-xs" />
              </div>
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Scale slutt</Label>
                <Input type="number" step="0.01" min="1" value={animationAdvanced.panScaleEnd} onChange={e => updateAnimationAdvanced({ panScaleEnd: Number(e.target.value || 1.12) })} className="h-8 text-xs" />
              </div>
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Pan X start (%)</Label>
                <Input type="number" step="0.1" value={animationAdvanced.panXStart} onChange={e => updateAnimationAdvanced({ panXStart: Number(e.target.value || -1.5) })} className="h-8 text-xs" />
              </div>
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Pan X slutt (%)</Label>
                <Input type="number" step="0.1" value={animationAdvanced.panXEnd} onChange={e => updateAnimationAdvanced({ panXEnd: Number(e.target.value || 1.5) })} className="h-8 text-xs" />
              </div>
            </div>
          )}

          {content?.imageAnimation === 'cinematic-reveal' && (
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Y offset (px)</Label>
                <Input type="number" step="1" min="0" value={animationAdvanced.revealYOffsetPx} onChange={e => updateAnimationAdvanced({ revealYOffsetPx: Number(e.target.value || 14) })} className="h-8 text-xs" />
              </div>
              <div className="space-y-1">
                <Label className="text-[11px] text-muted-foreground">Scale start</Label>
                <Input type="number" step="0.01" min="1" value={animationAdvanced.revealScaleStart} onChange={e => updateAnimationAdvanced({ revealScaleStart: Number(e.target.value || 1.03) })} className="h-8 text-xs" />
              </div>
            </div>
          )}

          {content?.imageAnimation === 'soft-float' && (
            <div className="space-y-1">
              <Label className="text-[11px] text-muted-foreground">Amplitude (px)</Label>
              <Input type="number" step="1" min="1" value={animationAdvanced.floatAmplitudePx} onChange={e => updateAnimationAdvanced({ floatAmplitudePx: Number(e.target.value || 8) })} className="h-8 text-xs" />
            </div>
          )}
        </div>
      )}
      <div className="flex items-center justify-between rounded-md border p-3">
        <Label className="text-xs">Vis bildetekst</Label>
        <Switch
          checked={content?.showCaptions !== false}
          onCheckedChange={(checked) => onUpdate({ ...content, showCaptions: checked })}
        />
      </div>
