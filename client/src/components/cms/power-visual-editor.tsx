...
import { TIDUM_TOKENS } from "@/lib/tidum-page-styles";
import "@/lib/tidum-page-styles.css";
...
function getGalleryPresentationStyles(imagePresentation?: string) {
  switch (imagePresentation) {
    case 'cinematic':
      return {
        figureClassName: 'rounded-xl border border-white/20 overflow-hidden bg-black shadow-lg',
        imageClassName: '[filter:saturate(0.9)_contrast(1.08)_brightness(0.95)]',
        captionClassName: 'px-3 py-2 text-[11px] tracking-[0.08em] uppercase bg-black/75 text-gray-200',
      };
    case 'soft':
      return {
        figureClassName: 'rounded-2xl border border-white/40 overflow-hidden bg-white/80 shadow-md',
        imageClassName: '[filter:saturate(1.05)_brightness(1.02)]',
        captionClassName: 'px-3 py-2 text-xs text-muted-foreground bg-white/80',
      };
    case 'editorial':
      return {
        figureClassName: 'rounded-none border-y border-border overflow-hidden bg-white',
        imageClassName: '[filter:contrast(1.04)_grayscale(0.08)]',
        captionClassName: 'px-3 py-2 text-[11px] uppercase tracking-[0.06em] text-muted-foreground bg-background/90',
      };
    default:
      return {
        figureClassName: 'rounded-xl border border-border overflow-hidden bg-muted/40',
        imageClassName: '',
        captionClassName: 'px-3 py-2 text-xs text-muted-foreground bg-background/80',
      };
  }
}

function getGalleryAnimationStyles(imageAnimation?: string) {
  switch (imageAnimation) {
    case 'cinematic-pan':
      return {
        imageClassName: 'animate-[tidum-cinematic-pan_14s_ease-in-out_infinite_alternate]',
      };
    case 'cinematic-reveal':
      return {
        imageClassName: 'animate-[tidum-cinematic-reveal_0.9s_ease-in-out_both]',
      };
    case 'soft-float':
      return {
        imageClassName: 'animate-[tidum-soft-float_14s_ease-in-out_infinite]',
      };
    default:
      return {
        imageClassName: '',
...
function getGridColsClass(columns: number, max = 3) {
  const bounded = Math.max(1, Math.min(columns || 1, max));
  ...
  return map[bounded] || 'grid-cols-1';
}
...
function SortableSection({ section, isSelected, onSelect, onUpdate, onDelete, onDuplicate, onCopy }: {
  ...
  const { attributes, listeners, setNodeRef, isDragging } = useSortable({ id: section.id });
    ...
    <div className="h-screen flex flex-col bg-background">
                ...
                {Object.entries(THEME_PRESETS).map(([key, theme]) => (
                  ...
                  >
                    <div className="flex gap-0.5">
                      <div className="h-4 w-4 rounded-full border bg-primary" />
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-auto bg-muted/30 p-8">
          <div className={`mx-auto w-full transition-all ${viewMode === 'mobile' ? 'max-w-[390px]' : viewMode === 'tablet' ? 'max-w-[768px]' : 'max-w-full'}`}>
                ...
                sections.map((section) => (
                  ...
                  >
                    ...
                    {section.content?.cards && Array.isArray(section.content.cards) && (
                      <div className={`mt-4 grid gap-2 ${getGridColsClass(section.content.cards.length, 3)}`}>
                    ...
                    {section.content?.benefits && Array.isArray(section.content.benefits) && (
                      <div className={`mt-4 grid gap-2 ${getGridColsClass(section.content.benefits.length, 3)}`}>
                    ...
                    {section.content?.stats && Array.isArray(section.content.stats) && (
                      <div className={`mt-4 grid gap-4 ${getGridColsClass(section.content.stats.length, 4)}`}>
                    ...
                    {section.content?.columns && Array.isArray(section.content.columns) && (
                      <div className={`mt-4 grid gap-4 ${getGridColsClass(section.content.columns.length, 4)}`}>
                    ...
                    {section.content?.primaryButton && (
                      <div className="mt-3 flex gap-2">
                        <div className={`rounded-lg bg-white/90 px-4 py-2 text-sm font-medium ${section.background.color === TIDUM_TOKENS.colorPrimary ? 'text-primary' : 'text-white'}`}>
                          {section.content.primaryButton.text}
                        </div>
                        {section.content?.secondaryButton && (
                          <div className={`rounded-lg border border-white/40 px-4 py-2 text-sm font-medium ${section.background.color === TIDUM_TOKENS.colorPrimary ? 'text-white' : 'text-foreground'}`}>
                    ...
                    {section.content?.fields && Array.isArray(section.content.fields) && (
                      <div className="mt-4 grid gap-2 grid-cols-2">
                        {section.content.fields.slice(0, 4).map((f: any, i: number) => (
                          <div key={i} className="rounded-xl border border-border bg-white/80 p-2 text-xs text-muted-foreground">
                            {f.label}
                          </div>
                        ))}
                    ...
                    {section.content?.plans && Array.isArray(section.content.plans) && (
                      <div className={`mt-4 grid gap-3 ${getGridColsClass(section.content.plans.length, 3)}`}>
                        {section.content.plans.map((plan: any, i: number) => (
                          <div key={i} className={`rounded-2xl border p-4 text-sm ${plan.highlighted ? 'border-primary ring-2 ring-primary/20 bg-primary/5' : 'border-border bg-white/80'}`}>
                            <div className="font-semibold text-base">{plan.name}</div>
                            <div className="text-2xl font-bold mt-1 text-primary">{plan.price}<span className="text-xs font-normal text-muted-foreground">{plan.period}</span></div>
                    ...
                    {section.content?.members && Array.isArray(section.content.members) && (
                      <div className={`mt-4 grid gap-3 ${getGridColsClass(section.content.members.length, 3)}`}>
                        {section.content.members.map((m: any, i: number) => (
                          <div key={i} className="rounded-2xl border border-border bg-white/80 p-4 text-center">
                            <div className="h-14 w-14 rounded-full mx-auto mb-2 flex items-center justify-center text-2xl bg-muted">
                              {m.name?.[0] || '?'}
                            </div>
                    ...
                    {section.content?.logos && Array.isArray(section.content.logos) && (
                      <div className="mt-4 flex flex-wrap gap-6 justify-center items-center">
                        {section.content.logos.map((logo: any, i: number) => (
                          <div key={i} className="rounded-xl border border-border bg-white/80 px-6 py-3 text-sm font-medium">
                            {logo.name}
                          </div>
                        ))}
                      </div>
                    ...
                    {section.content?.images && Array.isArray(section.content.images) && (
                      <div className={`mt-4 grid gap-2 ${getGridColsClass(section.content.cols || 3, 6)}`}>
                        {section.content.images.map((img: any, i: number) => {
                          const presentation = getGalleryPresentationStyles(section.content.imagePresentation);
                          const motion = getGalleryAnimationStyles(section.content.imageAnimation);
                          const aspectClass = getAspectRatioClass(section.content.aspectRatio);
                          const imageFitClass = section.content.imageFit === 'contain' ? 'object-contain bg-muted/30' : 'object-cover';

                          return (
                            <figure
                              key={i}
                              className={presentation.figureClassName}
                            >
                              {img.src ? (
                                <img
                                  src={img.src}
                                  alt={img.alt || `Galleri-bilde ${i + 1}`}
                                  className={`w-full ${aspectClass} ${imageFitClass} ${presentation.imageClassName} ${motion.imageClassName}`}
                                />
                              ) : (
                                <div
                                  className={`w-full flex items-center justify-center text-xs text-muted-foreground ${aspectClass}`}
                                >
                                  Bilde mangler
                                </div>
                              )}
                              {section.content.showCaptions !== false && (img.caption || img.alt) && (
                                <figcaption className={presentation.captionClassName}>
                                  {img.caption || img.alt}
                                </figcaption>
                              )}
                            </figure>
                    ...
                    {section.content?.videoUrl && (
                      <div className={`mt-4 rounded-xl border border-border overflow-hidden ${getAspectRatioClass(section.content.aspectRatio)}`}>
                        ...
                        {section.content.events.map((ev: any, i: number) => (
                          <div key={i} className="flex gap-4 items-start">
                            <div className="shrink-0 flex flex-col items-center">
                              ...
                              {i < section.content.events.length - 1 && <div className="w-0.5 h-8 bg-border" />}
                            </div>
                            <div className="pb-2">
                              <div className="text-xs font-semibold text-primary">{ev.date}</div>
                    ...
                    {section.content?.buttonText && section.content?.placeholder && (
                      <div className="mt-4 flex gap-2 max-w-md mx-auto">
                        <div className="flex-1 rounded-xl border border-border bg-white/80 px-4 py-2 text-xs text-muted-foreground">{section.content.placeholder}</div>
                        <div className="rounded-xl bg-primary px-5 py-2 text-xs font-medium text-white">{section.content.buttonText}</div>
                      </div>
                    )}
                  </div>
                ))
          ...
          <div className="w-80 border-l bg-card overflow-y-auto">
            ...
            <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)} className="p-4">
              ...
              <TabsContent value="design" className="space-y-4 mt-4">
                <div className="space-y-2">
                  <Label htmlFor="section-background-color">Bakgrunnsfarge</Label>
                ...
                {/* Text Color */}
                <div className="space-y-2">
                  <Label htmlFor="section-text-color">Tekstfarge</Label>
                  <div className="flex gap-2">
                    <input
                ...
                <div className="space-y-2">
                  ...
                  {(selectedSection.borderWidth || 0) > 0 && (
                    <div className="flex gap-2">
                      <Label htmlFor="section-border-color" className="sr-only">Kantfarge</Label>
                      <input
                        ...
                        title="Velg kantfarge"
                        aria-label="Velg kantfarge"
                      />
                      <Input
